...
PAYMENTS_FILE = getenv("PAYMENTS_FILE")
...
url = f"{DTONE_API_URL}/async/transactions"

# Load the data from the Excel spreadsheet
excel_file_path = PAYMENTS_FILE
df = pd.read_excel(excel_file_path, sheet_name="Payments to Make")
...
for index, row in df.iterrows():
    ...
    if make_payment:
        ...
        payload = {
            ...
            "credit_party_identifier": {"mobile_number": mobile_number},
            "auto_confirm": True,
        }

        # Making the POST request with HTTP Basic Authentication
        print(
            f"Processsing: number: {mobile_number}, external_id:{external_id}, product_id:{product_id}"
        )
        response = requests.post(
            url, json=payload, auth=HTTPBasicAuth(API_KEY, API_SECRET)
        )
        ...
        if response.status_code == 201:
            ...
            print(
                f"Payment request sent successfully for {external_id}, phone number {mobile_number}. Response ID: {response_message['id']}"
            )
        else:
            status = "Failed"
            print(response.text)
            ...
            print(
                f"Failed to send payment request for {external_id}: {response.status_code}, phone number {mobile_number}"
            )
...
# Save the log file
log_file_path = TRANSACTIONS_FILE  # Define the path where you want to save the log
